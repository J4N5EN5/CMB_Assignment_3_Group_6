...
def data_preprocessing(df):
    df.drop(['af', 'proto', 'step', 'type', 'fw', 'mver', 'dst_name', 'stored_timestamp', 'size', 'lts', 'src_addr',
             'from', 'result', 'Latitude', 'Longitude'], axis=1, inplace=True)
    return df


def pipeline_part1(df):
    return None


def pipeline_part2(df):
    return None


def pipeline_part3(df):
    return None
...
def main():
    ...
    pipeline_part1(df_results_preprocessed)
    pipeline_part2(df_results_preprocessed)
    pipeline_part3(df_results_preprocessed)
